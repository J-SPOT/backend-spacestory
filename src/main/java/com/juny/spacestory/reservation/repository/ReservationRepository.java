...
public interface ReservationRepository extends JpaRepository<SpaceReservation, Long> {
  ...
  @Query(
      "SELECT r FROM SpaceReservation r JOIN FETCH r.space WHERE r.space.id = :spaceId AND r.reservationDate = :reservationDate")
  List<SpaceReservation> findReservationsBySpaceIdAndDateJPQL(
      Long spaceId, LocalDate reservationDate);
}
