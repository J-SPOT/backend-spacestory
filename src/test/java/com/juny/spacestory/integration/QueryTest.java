...
public class QueryTest {
  ...
  void checkFindSpacesByCriteria() {
    ...
    Page<Space> byCriteriaQuerydsl =
        spaceRepository.findByCriteriaQuerydsl(
            SpaceType.FRIENDSHIP,
            "서울특별시",
            "강남구",
            1,
            Set.of(DetailedType.CAFE, DetailedType.RESIDENCE),
            pageRequest);
    System.out.println(
        "byCriteriaQuerydsl.getTotalElements() = "
            + byCriteriaQuerydsl
                .getTotalElements()); // 세부사항 없이 24개, CAFE 9개, RESIDENCE 6개, PARTYROOM 9개
  ...
  void checkQuerydsl() {
    ...
    Page<Space> bySpaceTypeInSeoul =
        spaceRepository.findBySpaceTypeInSeoul(SpaceType.FRIENDSHIP, pageRequest);
    System.out.println(
        "bySpaceTypeInSeoul.getTotalElements() = " + bySpaceTypeInSeoul.getTotalElements());

    Page<Space> bySpaceTypeInSeoulQuerydsl =
        spaceRepository.findBySpaceTypeInSeoulQuerydsl(SpaceType.FRIENDSHIP, pageRequest);
    System.out.println(
        "bySpaceTypeInSeoulQuerydsl.getTotalElements() = "
            + bySpaceTypeInSeoulQuerydsl.getTotalElements());
  }
}
